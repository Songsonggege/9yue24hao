...
from w3lib import html as h

""" 上海市虹口区城市管理行政执法局-行政处罚"""


class CfxkZyz0412014Spider(scrapy.Spider):
    name = 'CFXK-ZYZ-0412-014'
    allowed_domains = ['http://218.242.71.245']
    start_urls = ['http://http://218.242.71.245/']
    ...
    def start_requests(self):
        ...
        header = {'Referer': 'http://218.242.71.245/web/search.aspx?keyword=&type=1&organ=259de04e-88d9-4d35-8685-0c9c3d240e55'}

        for i in range(5):
            fromdata = {
                "Organ": "259de04e-88d9-4d35-8685-0c9c3d240e55",
                "SearchTypeValue": "1",
                "PageIndex": str(i)
            }
    ...
    def parse_detail(self, response: Response):
        ...
        # 设置正则匹配正文中的行政主体
        prPrincipal_condition = r"<p>([^:<>]{0,10}(经营者名称|自然人姓名|相对人名称|社团名称|被处罚单位名称|企业名称|被处罚[^:<>]{0,5}名称|当事人)[^:<>]{0,10}[：:])[^<>]{2,50}?(。|</p>)|=<p>(姓名|名称)[：:][^无<>]{2,50}?</p>|(当.{0,2}事.{0,2}人.{0,4}|被.{0,3}罚.{0,3}(单位|人).{0,5}(姓名|名称)?|[企单].{0,2}[位业].{0,2}名.{0,2}称)[:：]\\s?[^无<>]*?([\\s，性地。；]|</p>|经营地址|住址)"
        ...
        for para in paragraphList:
            ...
            para = re.sub(r"：", ":", para)
            para = h.replace_escape_chars(para)
            result_object = {
                "name": name,
                "createdAt": date,
                "publisher": publisher,
